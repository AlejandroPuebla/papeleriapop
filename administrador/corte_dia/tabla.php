...
$tabla="";
$query="SELECT i.id_producto,i.nombre_producto,v.hora,i.precio_unitario,v.cantidad,SUM(v.cantidad*i.precio_unitario) as total 
FROM ventas v, inventario i 
...
if(isset($_POST['inventario']))
{
	...
	$query="SELECT i.id_producto,i.nombre_producto,v.hora,v.cantidad,i.precio_unitario,SUM(v.cantidad*i.precio_unitario) as total 
			FROM ventas v, inventario i 
			WHERE v.id_producto = i.id_producto 
			AND v.fecha LIKE '%".$q."%' 
			GROUP BY v.id_producto";
...
if ($buscarInventario->num_rows > 0)
{
	...
	'<table class="table" class="table-data>
		<tr class="data names">
			...
			<th class="data-title">Hora De Venta</th>
